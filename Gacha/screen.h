...
void MR_PEEBLES(bool& mr_peebles, bool& peebles_switch, Sound peebles, Texture BG_GAME,
    Texture box2, Texture sprite, int SCREEN_WIDTH, int SCREEN_HEIGHT, Rectangle* frames, int currentFrame) {
    if (mr_peebles == true && peebles_switch == true) {
        PlaySound(peebles);
        peebles_switch = false;
        while (IsSoundPlaying(peebles)) {   
            BeginDrawing();
